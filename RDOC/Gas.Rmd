...
AT$Naam <- rep("Oostenrijk", nrow(AT))
...
CZ$Historical_Mean <- round(CZ$Historical_Mean, 2)
CZ <- subset(CZ, Maand_Dag != "02-29")
CZ$Naam <- rep("Tsjechië", nrow(CZ))
...
DE$Historical_Mean <- round(DE$Historical_Mean, 2)
DE <- subset(DE, Maand_Dag != "02-29")
DE$Naam <- rep("Duitsland", nrow(DE))
...
DK$Historical_Mean <- round(DK$Historical_Mean, 2)
DK <- subset(DK, Maand_Dag != "02-29")
DK$Naam <- rep("Denemarken", nrow(DK))
...
ES$Historical_Mean <- round(ES$Historical_Mean, 2)
ES <- subset(ES, Maand_Dag != "02-29")
ES$Naam <- rep("Spanje", nrow(ES))
...
FR$Historical_Mean <- round(FR$Historical_Mean, 2)
FR <- subset(FR, Maand_Dag != "02-29")
FR$Naam <- rep("Frankrijk", nrow(FR))
...
GB$Historical_Mean <- round(GB$Historical_Mean, 2)
GB <- subset(GB, Maand_Dag != "02-29")
GB$Naam <- rep("United Kingdom", nrow(GB))
...
HR$Historical_Mean <- round(HR$Historical_Mean, 2)
HR <- subset(HR, Maand_Dag != "02-29")
HR$Naam <- rep("Hongarije", nrow(HR))
...
HU$Historical_Mean <- round(HU$Historical_Mean, 2)
HU <- subset(HU, Maand_Dag != "02-29")
HU$Naam <- rep("Hongarije", nrow(HU))
...
IT$Historical_Mean <- round(IT$Historical_Mean, 2)
IT <- subset(IT, Maand_Dag != "02-29")
IT$Naam <- rep("Italië", nrow(IT))
...
LV$Historical_Mean <- round(LV$Historical_Mean, 2)
LV <- subset(LV, Maand_Dag != "02-29")
LV$Naam <- rep("Letland", nrow(LV))
...
NL$Historical_Mean <- round(NL$Historical_Mean, 2)
NL <- subset(NL, Maand_Dag != "02-29")
NL$Naam <- rep("Nederland", nrow(NL))
...
PL$Historical_Mean <- round(PL$Historical_Mean, 2)
PL <- subset(PL, Maand_Dag != "02-29")
PL$Naam <- rep("Polen", nrow(PL))
...
PT$Historical_Mean <- round(PT$Historical_Mean, 2)
PT <- subset(PT, Maand_Dag != "02-29")
PT$Naam <- rep("Portugal", nrow(PT))
...
RO$Historical_Mean <- round(RO$Historical_Mean, 2)
RO <- subset(RO, Maand_Dag != "02-29")
RO$Naam <- rep("Roemenië", nrow(RO))
...
SE$Historical_Mean <- round(SE$Historical_Mean, 2)
SE <- subset(SE, Maand_Dag != "02-29")
SE$Naam <- rep("Zweden", nrow(SE))
...
SK$Historical_Mean <- round(SK$Historical_Mean, 2)
SK <- subset(SK, Maand_Dag != "02-29")
SK$Naam <- rep("Slowakije", nrow(SK))
...
git_commit(message = "Automated commit")
...
add(repo, "230102_gasvoorhanden_In_EUropa.Rproj")
